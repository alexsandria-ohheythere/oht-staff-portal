...
export async function generatePayslipPDF({ staff, run, periodStart, periodEnd }) {
  ...
  if (run.additionalPayment > 0) lineL('Additional Payment (Extra Shifts)', run.additionalPayment)
  lineL('Incentives', run.incentives)
  lineL('Overtime', run.overtime)
  lineL('Refund', run.refund)
  if (run.serviceCharge > 0) lineL('Service Charge', run.serviceCharge)
  ly += 4
...
export function buildPayslipRun({ saved, dailyRate = 0, absenceDays = 0, periodLabel = '' }) {
  ...
  const additionalPayment = num(saved.additional_payment)
  const incentives = num(saved.incentives)
  const overtime = num(saved.overtime)
  const refund = num(saved.refund)
  // Service Charge was missing from this shared payslip-PDF builder entirely — the on-screen
  // Command Center Payslips card and payroll_runs.net_pay both include it, but the downloaded
  // PDF (both apps use this same function) silently dropped it from Gross/Net Pay. Fixed 2026-09-15.
  const serviceCharge = num(saved.service_charge)
  const grossPay = basic + additionalPayment + incentives + overtime + refund + serviceCharge
  ...
  return { basic, additionalPayment, incentives, overtime, refund, serviceCharge, grossPay, sss, philhealth, pagibig, tax, totalGovDeductions, late, undertime, absence, totalAttendance, netPay, periodLabel }
}
