...
// Place at: app/portal/team/page.js
// ─────────────────────────────────────────────
...
const SHIFTS = [
  ...
  { id:'pm',  label:'PM',  time:'2PM–11PM',       color:'#2d5a8a', bg:'#e8f0fb', border:'#4a90c4' },
]
...
export default function TeamSchedulePage() {
  ...
  function getShiftStaff(dayIdx, shiftId) {
    ...
    return schedules
      ...
      .map(s => staff.find(x => x.id === s.staff_id))
      .filter(Boolean)
  }
        ...
        <div style={{ background:'white', borderBottom:'1px solid #e5e7eb', padding:'16px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:4, background:'white', border:'1px solid #e5e7eb', borderRadius:8, padding:'3px 6px' }}>
              <button onClick={() => setWeekOffset(w => w-1)} style={{ width:28, height:28, border:'none', background:'transparent', cursor:'pointer', fontSize:16, color:'#6b7280', borderRadius:6 }}>‹</button>
              <button onClick={() => setWeekOffset(0)} style={{ fontSize:10, fontWeight:700, padding:'3px 8px', border:'1px solid #e5e7eb', borderRadius:6, background:'transparent', cursor:'pointer', color:'#6b7280' }}>Today</button>
              <button onClick={() => setWeekOffset(w => w+1)} style={{ width:28, height:28, border:'none', background:'transparent', cursor:'pointer', fontSize:16, color:'#6b7280', borderRadius:6 }}>›</button>
            </div>
          </div>
        </div>
          ...
          <div style={{ margin:'16px 20px 0', background:'#EF4576', borderRadius:12, padding:'12px 16px' }}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {myShifts.map((s, i) => {
                ...
                return (
                  <div key={i} style={{ background:isToday?'white':'rgba(255,255,255,.2)', borderRadius:8, padding:'6px 12px' }}>
                    ...
                    <div style={{ fontSize:10, color:isToday?'#6b7280':'rgba(255,255,255,.7)', marginTop:1 }}>{s.shift?.label} · {s.shift?.time}</div>
                  </div>
                )
              })}
            </div>
                ...
                {weekDates.map((date, di) => {
                  ...
                  return (
                    <div key={di}
                      ...
                      style={{ background:'white', borderRadius:12, border:isToday?'2px solid #EF4576':'1px solid #e5e7eb', cursor:'pointer', overflow:'hidden' }}
                      ...
                      <div style={{ padding:8 }}>
                        {dayShifts.length === 0
                          ? <p style={{ fontSize:10, color:'#d1d5db', textAlign:'center', padding:'12px 0', margin:0 }}>No shifts</p>
                          : dayShifts.map(shift => {
                              ...
                              return (
                                <div key={shift.id} style={{ marginBottom:6, background:isMine?shift.bg:'#f9fafb', border:`1px solid ${isMine?shift.border:'#f3f4f6'}`, borderRadius:8, padding:'6px 8px' }}>
                                  <div style={{ fontSize:9, fontWeight:700, color:shift.color, letterSpacing:1, textTransform:'uppercase', marginBottom:4 }}>{shift.label}{isMine?' ✓':''}</div>
                                  <div style={{ display:'flex', flexWrap:'wrap', gap:3 }}>
                                    {members.map(m => (
                                      <div key={m.id} title={`${m.first_name} ${m.last_name} · ${m.role}`}
                                        style={{ width:22, height:22, borderRadius:'50%', background:getRoleColor(m.role), display:'flex', alignItems:'center', justifyContent:'center', fontSize:8, fontWeight:700, color:'white', border:m.id===myStaffId?'2px solid #EF4576':'2px solid white', flexShrink:0 }}>
